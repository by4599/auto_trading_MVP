...
| 항목 | 내용 |
|---|---|
| Sleeve A (장기 추세추종) | **구현 보류.** 코드 없음, 자금 배정 없음 |
| Sleeve B (당일 모멘텀 단타) | 기존 `VolatilityBreakoutStrategy` 코어. 최대 자금 **30%** 한도 |
| 유휴 자금 | Sleeve A 미구현 기간 동안 70~100%는 **현금(CMA/예수금 이자)으로만** 대기. KOFR 등 파킹형 ETF 포함 모든 대체 포지션 운용 금지 (T+2 결제 지연이 진입 타이밍과 충돌하므로) |

> Sleeve A 구현이 시작되는 시점, 이 조항은 반드시 개정되어야 하며 그전까지 신규 제안에서
> Sleeve A에 자금이나 로직을 슬쩍 얹는 시도는 이 ADR 위반으로 간주한다.

### 2.2. 이원화된 자산 방어선과 `peakEquity` 생명주기

- **당일 손익 가드 (`DailyLossRule`)**: 매일 08:30 리셋, -3%(신규매수 차단) / -5%(강제청산 트리거) 2단계.
- **누적 자산 가드 (`Global Equity Stop`)**: 전고점 대비 MDD > 10%. 리셋 없음, 영구 추적.
