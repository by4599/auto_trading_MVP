...
- 지갑 칸 실험 (2026-07-19, `com.trading.bucket`): 방식별 자금 칸 분리 —
  ...
  **3방식 전부 실전 승격 불가(게이트 G2 미발동)** — 파이프라인 검증 목적의
  ...
  ⚠ **회귀 앵커는 781이다 (2026-08-17 재측정)** — 최초 기록(07-22·07-24)은 캔들이 판정 창 끝보다
  2거래일 모자란 상태에서 찍혀 780·총수익 129.36%였다. 완전한 데이터로는 **781·124.28%**이며
  PF·MDD·§4 판정은 불변. 원인은 `CandleBackfillService`의 7일 백필 슬랙(BACKLOG 등재,
  근거 `_workspace/7_quant_baseline-drift.md`). 옛 780을 대조 기준으로 쓰지 말 것.
