 ...
 *   (종목 수 × 이벤트 수)가 아니라 <b>이벤트 수</b>다. 계산 규칙은 {@link CrashVolCalculations}.
 *
...
public class LowVolCrashBacktester {
    ...
    public CrashVolReport compute(List<String> universe, LocalDate from, LocalDate to) {
        ...
        List<Double> anchorScope = kospiCloses.subList(
                0, CrashVolCalculations.anchorScopeSize(kospiDates, to));

        Map<Double, Integer> thresholdCounts = new LinkedHashMap<>();
        for (double t : THRESHOLD_GRID) {
            thresholdCounts.put(t, CrashVolCalculations
                    .detectAnchorIndices(anchorScope, t, TRAILING_DAYS, COOLDOWN_DAYS).size());
        }
        List<Integer> anchors = CrashVolCalculations
        ...
        for (VolWindow w : VOL_WINDOWS) {
            CrashVolScope scope = new CrashVolScope(kospiCloses, kospiDates, series, w,
                    MIN_BUCKET_COVERAGE, HORIZONS);
            windows.add(new WindowStats(w, CrashVolCalculations.aggregate(anchors, scope)));
        }
     ...
     * 앵커 탐지는 {@link CrashVolCalculations#anchorScopeSize}로 {@code to}까지 잘라 쓴다(재현성).
     */
    ...
    /** 한 종목의 종가 시계열 + 날짜→인덱스 맵 (집계가 목 없이 받도록 in-memory 표현) */
